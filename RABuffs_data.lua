...
function RAB_DefaultQueryHandler(query, cmd, needraw, needtxt)
 local buffed, fading, total, misc, txthead, hashead, txt, hastxt, invert, raw, rawsort, rawgroup = 0,0,0, "", "", "", "","", (RAB_Buffs
...
[cmd].invert ~= nil);
 ...
 for i, u, group in RAB_GroupMembers(query) do
  ...
  if (RAB_IsEligible(u,cmd)) then
	...
	if (needraw or needtxt) then
		tinsert(raw, {unit=u, name=UnitName(u) .. ((appl ~= nil and appl > 1) and " [" .. appl .. "]" or ""), class=RAB_UnitClass

(u), group=group, buffed=isbuffed,fade=fadetime});
	end
 ...
 if (table.getn(raw) > 0) then
  ...
  for i=1,table.getn(raw) do
   if (ident ~= raw[i][rawsort] and ident ~= false) then
    if (uc == 0) then
     if (bc > 1) then
	hastxt = hastxt .. (hastxt ~= "" and ", " or "") .. (rawsort == "group" and sRAB_BuffOutput_Group or "") .. ident .. (rawsort == 
...
"class" and "s" or "") .. " [" .. bc .. "]";
    ...
    elseif (bc == 0) then
     if (uc > 1) then
	txt = txt .. (txt ~= "" and ", " or "") .. (rawsort == "group" and sRAB_BuffOutput_Group  or "") .. ident  .. (rawsort == "class" 
...
and "s" or "") .. " [" .. uc .. "]";
  ...
  if (uc == 0) then
   hastxt = hastxt .. (hastxt ~= "" and ", " or "") .. (rawsort == "group" and sRAB_BuffOutput_Group  or "") .. ident .. (rawsort == "class" 

and "s" or "")  .. " [" .. bc .. "]";
  elseif (bc == 0) then
   txt = txt .. (txt ~= "" and ", " or "") .. (rawsort == "group" and sRAB_BuffOutput_Group  or "") .. ident .. (rawsort == "class" and "s" 
...
function RAB_QueryBuffInfo()
	...
	local text = string.format(sRAB_BuffOutput_BuffInfo_General,UnitName("target"),(buffs == "" and sRAB_BuffOutput_BuffInfo_None or 

buffs), (debuffs == "" and sRAB_BuffOutput_BuffInfo_None or debuffs));
	return 0,0,0,"","","",text,text;
end
function RAB_QueryHere(msg,needraw,needtxt)
	local buffed, total, misc, txthead, hashead, txt, hastxt, invert, raw, rawsort = 0,0, "", sRAB_BuffOutput_Here_NotHere, 

sRAB_BuffOutput_Here_Here, sRAB_BuffOutput_Here_NotHere .. " ",sRAB_BuffOutput_Here_Here .. " ", false, nil, "append";
...
function RAB_QueryCTRAVersion(msg, needraw, needtxt)
	local buffed, total, misc, txthead, hashead, txt, hastxt, invert, raw, rawsort = 0,0, "", sRAB_BuffOutput_CTRA_OutOfDate, 

sRAB_BuffOutput_CTRA_Recent, sRAB_BuffOutput_CTRA_OutOfDate .. " ",sRAB_BuffOutput_CTRA_Recent .. " ", false;
	...
	if (not UnitInRaid("player")) then
		...
		for i,u,group in RAB_GroupMembers(msg) do
			...
			if (cv) then
				...
				if (needraw) then tinsert(raw, {name=UnitName(u),class=RAB_UnitClass(u),group=group,buffed=isbuffed,unit=u, 
...
function RAB_QueryHealth(msg, needraw, needtxt)
	...
	local buffed, fading, total, misc, txthead, hashead, txt, hastxt, invert, raw, rawsort = 0,0,0, "", sRAB_BuffOutput_Health_Dead, 

sRAB_BuffOutput_Health_Alive,"","", false;
	...
	for i,u,group in RAB_GroupMembers(msg) do
		...
		if (needraw) then tinsert(raw, {name=UnitName(u),class=RAB_UnitClass(u),group=group,buffed=isbuffed,unit=u,append=append}); 
	...
	if (needtxt) then
		if (hp_dead > 0) then
			if (hp_alive > 0) then
				txt = string.format(sRAB_BuffOutput_Health_Default,hp_cur, hp_max, floor(hp_cur*100/(hp_max > 0 and hp_max 

or 1)), string.format(sRAB_BuffOutput_Health_DeadPart,hp_dead, hp_deadtext));
				hastxt = string.format(sRAB_BuffOutput_Health_Default,hp_cur, hp_max, floor(hp_cur*100/(hp_max > 0 and 

hp_max or 1)), string.format(sRAB_BuffOutput_Health_AlivePart,hp_alive, hp_alivetext));
			else
...
function RAB_QueryMana(msg, needraw, needtxt)
	local buffed, fading, total, misc, txthead, hashead, txt, hastxt, invert, raw, rawsort = 0,0,0, "", sRAB_BuffOutput_Mana_OutOfMana, 

sRAB_BuffOutput_Mana_Fine,"","", false;
...
append=append}); end
	...
	if (needtxt) then
		txt = "Mana: " .. mana_cur .. " / " .. mana_max .. " (" .. (mana_max > 0 and floor(mana_cur*100/mana_max) or "0") .. "%); " 
...
function RAB_DefaultCastingHandler(mode, query)
 ...
 for i, u, g in RAB_GroupMembers(query) do
  ...
  if (RAB_Buffs[cmd].priority ~= nil and RAB_Buffs[cmd].priority[strlower(RAB_UnitClass(u))] ~= nil) then pri = pri + RAB_Buffs

[cmd].priority[strlower(RAB_UnitClass(u))]; end
 ...
 if (dogroupbuff and table.getn(people) < RAB_Buffs[cmd].bigthreshold and not RABui_Settings.alwayscastbigbuffs) then dogroupbuff = false; 
  ...
  if (bsort[1].heads >= RAB_Buffs[cmd].bigthreshold or RABui_Settings.alwayscastbigbuffs) then
   ...
   if (mode == "cast") then
    ...
    if (RAB_CastSpell_Start(RAB_Buffs[cmd].bigcast)) then
     ...
     RAB_Print(string.format(sRAB_CastBuff_Cast, bname, RAB_Chat_Colors[RAB_UnitClass(bsort[1].cast)] .. UnitName(bsort[1].cast) .. " [" .. 
...
bsort[1].key .. "]"));
    ...
    return string.format(clicktocast, bname, RAB_Chat_Colors[RAB_UnitClass(bsort[1].cast)] .. UnitName(bsort[1].cast) .. " [" .. bsort
...
[1].key .. "]");
 ...
 if (mode == "cast") then
  ...
  RAB_Print(string.format(sRAB_CastBuff_Cast, sRAB_SpellNames[cmd] ~= nil and sRAB_SpellNames[cmd] or RAB_Buffs[cmd].name, RAB_Chat_Colors

[RAB_UnitClass(people[1].u)] .. UnitName(people[1].u)));
  return true;
...
function RAB_CastResurrect(mode, msg)
 ...
 for i, u, group in RAB_GroupMembers(msg) do
  if (UnitIsUnit(u,"player") ~= 1 and UnitIsVisible(u) and (RAB_UnitIsDead(u) and not UnitIsGhost(u)) and RAB_IsSanePvP(u) and 

(RAB_CTRA_IsBeingRessed == nil or not RAB_CTRA_IsBeingRessed(UnitName(u)))) then
   local pri = (RAB_UnitClass(u) == "Priest" and 2 or 0) + (RAB_UnitClass(u) == "Paladin" and 2 or 0) + (RAB_UnitClass(u) == "Shaman" and 2 

or 0) + (RAB_UnitClass(u) == "Mage" and 1 or 0) + (RAB_Versions[UnitName(u)] ~= nil and 0.25 or 0);
...
-- BLOCK 3: Query definitions
RAB_Buffs = 	{
			ai={name="Arcane Intellect",bigcast="ab",bigsort="group",bigthreshold=3,textures={"Spell_Holy_MagicalSentry", 

"Spell_Holy_ArcaneIntellect"},ignoreClass="wr",castClass="Mage",priority=
...
RABuffs.",ignoreClass="rw"},
			...
			pwf={name="Power Word: Fortitude",bigsort="group",bigthreshold=3,bigcast="pof",textures={"Spell_Holy_WordFortitude", 

"Spell_Holy_PrayerOfFortitude"},castClass="Priest",ctraid=1,recast=5},
...
{priest=0.5,druid=0.3,mage=0.4,paladin=0.3},ctraid=8,recast=5},
			...
			pi={name="Power Infusion",textures={"Spell_Holy_PowerInfusion"},castClass="Priest",ignoreClass="wrh",priority=
...
{"Ability_Druid_TravelForm","Ability_Racial_BearForm","Ability_Druid_CatForm","Ability_Druid_AquaticForm"},type="dummy",castClass="Druid"},
...
{"Spell_Holy_FistOfJustice","Spell_Holy_GreaterBlessingofKings"},ignoreClass="mplh",castClass="Paladin",sort="class",ctraid=11,recast=3},
			bosanc={name="Blessing of Sanctuary",bigcast="gbosanc",bigsort="class",bigthreshold=3,textures=

{"Spell_Nature_LightningShield","Spell_Holy_GreaterBlessingofSanctuary"},castClass="Paladin",sort="class",ctraid=16,recast=3},
...
{priest=2,paladin=2,druid=1,mage=0.5},selfPriority=-10},
			...
			ss={name="Soulstone",textures={"Spell_Shadow_SoulGem"},castClass="Warlock",buffFunc=RAB_CastSoulstone,priority=
...
{"Spell_Nature_ProtectionformNature"},castClass="Hunter",type="aura"},
			...
			flag={name="WSG Flag",textures={"INV_BannerPVP_01","INV_BannerPVP_02"} ,havebuff="Carrying Flag",missbuff="No 
...
Flag"},
			...
			regen={name="Regenerating",textures={"INV_Drink_18","INV_Drink_07","INV_Misc_Fork&Knife"},castClass="Item" 

,havebuff="Regenerating",missbuff="Not Regenerating",type="self"},
			...
			tuber={name="Runn Tum Tuber Surprise",textures={"INV_Misc_Organ_03"},castClass="Food-like buffs" ,type="aura"},
                        squid={name="Squid Check",textures={"INV_Gauntlets_19"},castClass="Food-like buffs",invert=true,type="aura"},
                        ...
                        arcanerezprot={name="Arcane Resistance Potion",textures={"Spell_Holy_PrayerOfHealing02"},castClass="Resistance 

Potions",type="aura"},
                        naturerezprot={name="Nature Resistance Potion",textures={"Spell_Nature_SpiritArmor"},castClass="Resistance 

Potions",type="aura"},
                        firerezprot={name="Fire Resistance Potion",textures={"Spell_Fire_FireArmor"},castClass="Resistance 

Potions",type="aura"},
                        shadowrezprot={name="Shadow Resistance Potion",textures={"Spell_Shadow_RagingScream"},castClass="Resistance 
...
Potions",type="aura"},
                        ...
                        armorpot={name="Elixir of Superior Defense",textures={"INV_Potion_66"},castClass="Tanking Consumables" 
...
,type="aura"},
                        ...
                        jujumight={name="Juju Might",textures={"INV_Misc_MonsterScales_07"},castClass="Melee MISC" ,type="aura"},
                        mtrollpot={name="Mighty Troll Potion",textures={"INV_Potion_79"},castClass="Tanking Consumables" ,type="aura"},
                        nightfin={name="Nightfin Soup",textures={"Spell_Nature_ManaRegenTotem"},castClass="Food-like buffs" ,type="aura"},
...
{"INV_Jewelry_Talisman_07"},castClass="Item",invert=true,type="self"},
			...
			health={name="Health",type="special",queryFunc=RAB_QueryHealth,ext="hp",buffFunc=RAB_CastResurrect,description="Sum 

of health versus sum of max health."},
			alive=

{name="Alive",type="special",queryFunc=RAB_QueryHealth,ext="alive",buffFunc=RAB_CastResurrect,description="Number of people alive versus 

total headcount."},
			mana={name="Mana",type="special",queryFunc=RAB_QueryMana,description="Sum of current mana vs sum of max 

mana.",ignoreClass="rw"},
			status={name="Status",type="special",queryFunc=RAB_QueryStatus,description="Displays buff sumary for PW:F, AI, MotW, 

BoK, BoS, BoW and Soulstones.",noUI=true},
			scanunknown={name="Unknown Buff Scan",type="special",queryFunc=RAB_ScanRaid,ext="unknown",description="Scans raid 

for unknown buff textures.",noUI=true},
			scanraid={name="Raid Scan",type="special",queryFunc=RAB_ScanRaid,ext="known",description="Scans raid and displays a 

report of all known buffs.",noUI=true},
			ishere={name="Is Here",type="special",queryFunc=RAB_QueryHere,description="Displays people currently afk, offline or 

invisible."},
			ctra={name="CTRA Version",type="special",queryFunc=RAB_QueryCTRAVersion,description="Displays people whose CTRA is 

out of date."},
			blank={name="Blank",type="special",queryFunc=RAB_QueryBlank,description="Displays a blank bar - use as a header if 

you wish."},
			onycloak={name="Onyxia 

Cloak",type="special",queryFunc=RAB_QueryInventoryItem,ext="15:15138",buffFunc=RAB_CastInventoryItem,description="Checks that people are 

wearing their Onyxia Cloak."},
			info={name="Target's (De)Buffs",type="special",queryFunc=RAB_QueryBuffInfo,description="Outputs buff names / 

textures for buffs and debuffs on your current target.",noUI=true},
